...
class DuplicateBCPCampaignPledgesSeederForCalendarYear2025 extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        // table:  eligible_employee_by_bus
        $ee_by_bu = EligibleEmployeeByBU::where('campaign_year', 2024)
                                        ->where('organization_code', 'GOV')
                                        ->where('business_unit_code', 'BC088')
                                        ->first();

        EligibleEmployeeByBU::updateOrCreate(
           ['campaign_year' => $ee_by_bu->campaign_year,
            'as_of_date' => $ee_by_bu->as_of_date,
            'organization_code' => $to_org->code, 
            'business_unit_code' => $ee_by_bu->business_unit_code,
           ],[
            'business_unit_name' => $ee_by_bu->business_unit_name,
            'ee_count' => $ee_by_bu->ee_count,
            'notes' => $ee_by_bu->notes,
...
// ->whereIn('id', [763] )
        ...
        foreach ($pledges as $idx => $pledge) {
            ...
            $cancelled_count++;
            $pledge->cancelled = 'Y';
            $pledge->cancelled_by_id = 998;
            $pledge->cancelled_at = now();
            $pledge->save();

            echo PHP_EOL;
            echo '   SUCCESS - The old Gov pledge for Organization GOV was cancelled.' . ' - id ' . $pledge->id . PHP_EOL;
        ...
        echo 'Total cancelled : ' .  $cancelled_count . PHP_EOL;
