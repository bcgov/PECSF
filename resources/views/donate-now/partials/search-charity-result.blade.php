...
<table id="charities">
    ...
    @foreach($charities as $charity)
        <tr >
            ...
            <td style="width:5%"><div style="width:100px;" class="select-btn btn btn-outline-primary {{ $selected_charity_id == $charity->id ? 'active' : '' }}""
                    name="{{$charity->charity_name}}" org_id="{{$charity->id}}">
                        {{  $selected_charity_id == $charity->id ? 'Selected' : 'Select' }}</div></td>
        </tr>
    @endforeach
    </tbody>
</table>
