...
<table id="charities" class="col-md-12">
    @foreach($organizations as $key => $organization)
        ...
        <tr>
            ...
            <td class="blue" style="width:20%"><span class="text-primary view_details" pool_description="{{$organization->pool_description}}" pool_image="/img/uploads/fspools/{{$organization->image}}" charity_type="{{$organization->charity_type}}" charity_name="{{$organization->charity_name}}" charity_description="" registration_number="{{$organization->registration_number}}" charity_status="{{$organization->charity_status}}" effective_date_of_status="{{$organization->effective_date_of_status}}" sanction="{{$organization->sanction}}" designation="{{$organization->designation}}" category="{{$organization->category}}" address="{{$organization->address}}" city="{{$organization->city}}" province="{{$organization->province}}" country="{{$organization->country}}" postal="{{$organization->postal_code}}" website="{{$organization->uri}}" charitable_programs=""><b><u>View Details</u></b></span></td>
