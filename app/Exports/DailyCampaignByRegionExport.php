...
class DailyCampaignByRegionExport implements FromCollection, WithHeadings, WithColumnWidths, 
            WithStyles, ShouldAutoSize, WithColumnFormatting, WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        $total_row  = DailyCampaign::where('daily_type', 1)
                            ->where('campaign_year', $this->campaign_year)
                            ->where('as_of_date', $this->as_of_date)
                            ->select( 
                                DB::raw("'Total' as region_name"),
