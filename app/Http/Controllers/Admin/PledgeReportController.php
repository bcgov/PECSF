...
{
    //
    protected $process_name = 'PledgesExportJob';
    ...
    public function index(Request $request)
    {
        ...
        return view('admin-report.pledges.index', compact('years'));
    ...
    public function export2csv(Request $request) {
        ...
        if($request->ajax()) {
            ...
            // Submit a job 
            $batch = Bus::batch([
                new PledgesExportJob($history->id, $filename, $filters),
            ])->dispatch();
    ...
    public function exportProgress(Request $request, $id) {
        ...
        if ($history) {
            ...
            if ($batch->finished() ) {
                ...
                $link = route('reporting.pledges.download-export-file', $history->id);
