...
use App\Jobs\VolunteerProfilesExportJob;
...
class VolunteerProfileReportController extends Controller
{
    //
    protected $process_name = 'VolunteerProfilesExportJob';
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()) {
            ...
            return Datatables::of($processes)
                ...
                ->addColumn('download_file_link', function ($process) {
                    ...
                    if ($process->status == 'Completed') {
                        if ($process->updated_at >= today()->subdays($retention_days) ) {
                            $url = route('reporting.volunteer-profiles.download-export-file', $process->id);
                ...
                // ->addColumn('action', function ($process) {
                //     return '<a class="btn btn-info btn-sm" href="' . route('admin-volunteerprofile.campaign.show',$process->id) . '">Show</a>' .
                //         '<a class="btn btn-primary btn-sm ml-2" href="' . route('admin-volunteerprofile.campaign.edit',$process->id) . '">Edit</a>';
                // })
    ...
    public function export2csv(Request $request) {
        ...
        if($request->ajax()) {
            ...
            // Submit a job 
            $batch = Bus::batch([
                new VolunteerProfilesExportJob($history->id, $filename, $filters),
            ])->dispatch();
    ...
    public function exportProgress(Request $request, $id) {
        ...
        if ($history) {
            ...
            if ($batch->finished() ) {
                ...
                $link = route('reporting.volunteer-profiles.download-export-file', $history->id);
