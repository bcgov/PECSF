...
use App\Jobs\GamingAndFundrasingExportJob;

class GamingAndFundrasingReportController extends Controller
{
    //
    protected $process_name = 'GamingAndFundrasingExportJob';
    ...
    public function index(Request $request)
    {
                ...
                ->addColumn('download_file_link', function ($process) {
                    ...
                    if ($process->status == 'Completed') {
                        if ($process->updated_at >= today()->subdays($retention_days) ) {
                                $url = route('reporting.gaming-and-fundrasing.download-export-file', $process->id);
        ...
        $years = BankDepositForm::join('campaign_years', 'campaign_years.id', 'bank_deposit_forms.campaign_year_id')
                        // ->whereNull('cancelled')
                        ->distinct('campaign_years.calendar_year')->orderBy('campaign_years.calendar_year')->pluck('campaign_years.calendar_year');

        // load the view and pass data
        return view('admin-report.gaming-and-fundrasing.index', compact('years'));
    ...
    public function export2csv(Request $request) {
        ...
        if($request->ajax()) {
            ...
            $filename = 'Gaming_and_Fundrasing_'.now()->format("Y-m-d-his").".xlsx";
            ...
            // Submit a job 
            $batch = Bus::batch([
                new GamingAndFundrasingExportJob($history->id, $filename, $filters),
            ])->dispatch();
    ...
    public function exportProgress(Request $request, $id) {
        ...
        if ($history) {
            ...
            if ($batch->finished() ) {
                ...
                $link = route('reporting.gaming-and-fundrasing.download-export-file', $history->id);
