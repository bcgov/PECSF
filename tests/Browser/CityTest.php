...
it('redirects anonymous user to the login page when attempting to access the city', function () {
    $this->browse(function (Browser $browser) {

        $browser->visit('/settings/cities') 
...
it('denies unauthorized users from accessing protected city maintenance routes', function () {
    $this->browse(function (Browser $browser) {
...
it('administrator cannot access unavailable city maintenance routes', function () {
    $this->browse(function (Browser $browser) {
...
it('administrator can paginate through cities', function () {

    City::factory(25)->create();
    $sorted = City::orderBy('city')->get();

    $this->browse(function (Browser $browser) use($sorted) {
        $browser->loginAs($this->admin)
            ->visit('/settings/cities') // API endpoint
            // ->screenshot('debug-pagation-page') 

            ->waitForText(  'Showing 1 to 10' )
            ->assertSee( $sorted[0]->city ) // Check if "name" key exists in JSON
            ->assertSee( $sorted[0]->DescrShort ) // Check if "name" key exists in JSON
            ->click('a[data-dt-idx="2"]')
            ->waitForText(  'Showing 11 to 20' )
            ->assertSee( $sorted[10]->city ) // Ch
