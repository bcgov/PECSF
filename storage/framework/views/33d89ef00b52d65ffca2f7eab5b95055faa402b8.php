    ...
    <div class="row">
        <div class="col-md-12 offset-md-0">
            <div id="search">
                ...
                <form id="charity-searchform" action="<?php echo e(route('donate.save.select')); ?>" method="post">
                  <?php echo csrf_field(); ?>
                  <div class="input-group">
                        
                        <span class="position-relative w-100">
                          <input  id="charity-name" type="search" name="title" value="<?php echo e(request()->get('charity-name','')); ?>"
                               class="charity-name form-control bg-white" placeholder="Search for your CRA Charity"/>
                          <small id="charity-name-error" class="text-danger charity-name-error">
                  ...
                  <div id="search-charity" class="position-relative pb-3"  >
                    ...
                    <div id='search-filter' class="search-filter position-absolute w-100 p-2 bg-white" style="display:none; min-height:300px;">
                              ...
                              <div class="row">
                                  <div class="col-md-4">
                                    <select  class="form-control input-sm refresh_search_result" id="designation_code" name="designation_code">
                                      ...
                                      <?php $__currentLoopData = $designation_list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($key); ?>" <?php echo e(old('designation_code') == $value ?? 'selected'); ?>><?php echo e($value); ?></option>
                                    ...
                                    <div class="col-md-4">
                                      <select  class="form-control input-sm refresh_search_result" id="category_code" name="category_code">
                                        ...
                                        <?php $__currentLoopData = $category_list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                          <option value="<?php echo e($key); ?>" <?php echo e(old('category_code') == $value ?? 'selected'); ?>><?php echo e($value); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                      </select>
                                    </div>
                                    <div class="col-md-4">
                                      <select  class="form-control input-sm refresh_search_result" id="province" name="province">
                                        ...
                                        <option value="" <?php echo e(old('province') ? '' : 'selected'); ?>>Select Province</option>
                                        <?php $__currentLoopData = $province_list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                          <option value="<?php echo e($key); ?>" <?php echo e(old('province') == $value ?? 'selected'); ?>><?php echo e($value); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                      </select>
                      ...
                      <div class= "container-fluid p-0 m-0 ">
                        <div class="pagination_data" id="pagination_data"
                            style="min-heightXXX: 320px; ">
                          <?php echo $__env->make("donate.partials.charity-pagination",["charities"=>$charities], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="m-2 py-2 pl-2 bg-light">
                  <div id="selected-charity-list" class="selected-charity-list">
                  </div>
                </div>


                <div class="mt-2 toggle">
                  
                  <a class="btn btn-lg btn-outline-primary" href="<?php echo e(route('donate.start')); ?>">Previous</a>
...
<script>
    ...
    //GER
    r = "t";
    var multiple = "<?php echo e($multiple); ?>"
    $(function () {
        let selectedCharity = <?php echo json_encode($selected_charities); ?>;
        const $select = $("select");
        ...
        $(document).on("click", ".charity-select-add" , function(e) {
            ...
            if (selectedCharity.findIndex((c) => c.id === parseInt($(this).val())) === -1) {
              ...
              renderSelectedCharity();
              $('.search-filter').eq(window.lastIndex).hide();
              $('.charity-name').eq(window.lastIndex).val('');
            }
        });
        ...
        $('.charity-name').on('search', delay(function (e) {
              ajax_search_charities($('.charity-name').index($(this)));
        }, 250));

        $(document).on('keyup','.charity-name',delay(function (e) {

          $('.charity-name-error').eq(window.lastIndex).html('');
        ...
        function ajax_search_charities(index=0) {

              //get url and make final url for ajax
              var url = '<?php echo e(url("donate/select")); ?>';
              var append = url.indexOf("?") == -1 ? "?" : "&";
              var finalURL = url + append + "title="+$(".charity-name").eq(index).val();
              ...
              $.ajax({
                ...
                success: function(data,index) {
                  $(".pagination_data").eq(window.lastIndex).html(data);
                },
                complete: function() {
                  $(".loading-spinner").hide();
                  $('.search-filter').eq(window.lastIndex).show();
                }
              });
      }
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\Users\x257354\Sites\PECSF\resources\views/donate/partials/charity-search.blade.php ENDPATH**/ ?>